...
# 后台IP设置
导出 Ipv4_ipaddr="192.168.2.2"            # 修改openwrt后台地址(填0为关闭)
导出 Netmask_netm="255.255.255.0"         # IPv4 子网掩码（默认：255.255.255.0）(填0为不作修改)
导出 Op_name="0"                # 修改主机名称为OpenWrt-123(填0为不作修改)
...
# 默认主题设置
导出导出强制主题="0"              # 将bootstrap替换您需要的主题为必选主题(可自行更改您要的,源码要带此主题就行,填写名称也要写对) (填写主题名称,填0为不作修改)Mandatory_theme="0"              # 将bootstrap替换您需要的主题为必选主题(可自行更改您要的,源码要带此主题就行,填写名称也要写对) (填写主题名称,填0为不作修改)
导出导出 默认主题="0"
# 多主题时,选择某主题为默认第一主题 (填写主题名称,填0为不作修改)默认主题="0"


# 旁路由选项
导出导出 旁路由设置="0"                 # 旁路由设置 IPv4 网关(填入您的网关IP为启用)(填0为不作修改)旁路由设置="0"                 # 旁路由设置 IPv4 网关(填入您的网关IP为启用)(填0为不作修改)
...
- 确保所有的文件和目录都具有正确的权限，以便AdGuardHome能够读取和执行它们。
- 在编写插件和核心逻辑时，确保你的代码是异步的，并且使用了适当的错误处理机制。
- 如果你的插件或核心需要额外的资源文件，如图像或配置文件，你可以将它们放在相应的目录中，并在`info.json`文件中定义它们的路径。
导出 AdGuardHome_Core="1"                  # 编译固件时自动增加AdGuardHome插件和AdGuardHome插件核心,需要注意的是一个核心20多MB的,小闪存机子搞不来(1为启用命令,填0为不作修改)
